...
| `macos-statusbar` | Host is a Mac running NanoClaw as a host service | Green/red menu-bar dot + start/stop/restart + launch-on-login — directly softens "the session IS the system." **Applies as-is now** — it manages a launchd NanoClaw, which is exactly how this deployment runs since moving off `sbx`. Previously rejected pending adaptation to an sbx VM lifecycle; that blocker no longer exists |
