...
---
Pure housekeeping: commits this agent's history files — the social follower
series and the per-property GA4 traffic series — to a branch in the marketing
repo, so they outlive this container. **A successful publish never wakes
you**; you are only invoked when something failed.

**Why this one matters more than any other task's state.** The follower
series is the only thing in this entire system that is unrecoverable by
construction. Facebook, LinkedIn, Instagram and YouTube each expose the
*current* count and nothing else — there is no API, page, or export that will
tell you what the number was last Tuesday. Every day this task is broken is a
day that can never be reconstructed by anyone, at any cost. The GA4 traffic
files are re-queryable inside the property's retention window (14 months by
default) and permanently gone beyond it.

That makes a failure here worth telling your lead about the same day — not as
an emergency, but never silently swallowed either.

- **`not-configured`**: `LEDGER_REPO` (or `MARKETING_REPO`) isn't set. Say it
  once, plainly, including the consequence — the series is container-local and
  dies at the next rebuild — then stop re-raising it every run.
- **`nothing-to-publish`**: no history files exist yet. Normal on a fresh
  install; `social-metrics-snapshot` and `weekly-analytics-report` create them
  on their own schedules. Nothing to do.
- **`clone-failed`**: most likely `github.com` (the git host, distinct from
  ...
  credential with write access exists in the vault.
- **`push-failed`**: the commit is kept locally and retried next run, so one
  occurrence isn't data loss. Twice in a row points at the credential lacking
  write access to that repo.
- **`fetch-failed` / `checkout-failed` / `branch-create-failed`**: report the
  status verbatim. These refuse to force anything on purpose — an intact
  history is never worth overwriting to make a task report green.

**Never substitute posting the numbers into a channel for a failed publish.**
A follower count in a chat message is not a series; the file in the repo is.
