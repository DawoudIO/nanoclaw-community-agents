...
# The ledger repo is where ledger-publish commits the three history series.
...
if [ -z "${LEDGER_REPO:-}" ]; then
  add "config:LEDGER_REPO" "skipped" "optional but strongly recommended — without it ledger-publish cannot run, so the dev-metrics, traffic and follower series stay container-local and are lost at the next rebuild. Follower counts cannot be re-read retroactively from any platform."
else
  ...
  case "$CODE" in
    ...
    401|403) add "ledger_repo_access" "unreachable" "$LEDGER_REPO: token not wired or lacks access (401/403). ledger-publish also needs a github.com (git) PUSH credential, which this REST check cannot verify — confirm that by running the task once.";;
