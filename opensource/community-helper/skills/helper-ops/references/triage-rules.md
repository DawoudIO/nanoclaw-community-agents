...
- **Missing repro basics** — no steps, no version, no environment. Draft the
  question your manager can ask; don't leave the issue thin and hope.
...
- Don't apply labels unless the project's label scheme is completely
  unambiguous and your manager has explicitly delegated it.
...
`COMMUNITY_REPOS` lists the repos you actively triage — normally the ones with real external community activity, but it's the owner's call. If a repo you're expected to watch isn't in `COMMUNITY_REPOS`, ask your manager rather than assuming, and
...
- When a behavior-changing PR merges or a release ships in the product repo,
  ...
  to your manager — a release is not "done" while its docs issue is open.
...
- **When a release is imminent**: sweep for open docs/site PRs that should
  land with it and surface them to your manager — a release with its docs PRs
