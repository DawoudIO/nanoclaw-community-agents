...
script: |
  ...
  #   docs-gap-review (the MANAGER) — trigger: USERS keep asking the same thing.
  ...
  DOCS_REPO="${DOCS_REPO:-}"
  if [ -z "$DOCS_REPO" ]; then
    echo '{"wakeAgent": false, "data": {"status": "not-configured", "hint": "set DOCS_REPO (and optionally DOCS_PATH) in plugin-data/community-helper/config.env — the project has no docs target, so there is nowhere to send a docs PR"}}'
    exit 0
  fi
  ...
  if [ -z "$SRC" ]; then
    echo '{"wakeAgent": false, "data": {"status": "not-configured", "hint": "set PRODUCT_REPO or COMMUNITY_REPOS in plugin-data/community-helper/config.env"}}'
    exit 0
  fi
  DOCS_PATH="${DOCS_PATH:-}"
...
- **No milestone?** `without_milestone` counts these. Use `latest_release` to
  ...
  target version is unconfirmed — then ask your manager to get the version from
...
what changed without describing the diff, say so and hand it to your manager,
which has the better voice for user-facing text.

Report to your manager: what you drafted with links, what needed nothing, and any
