...
script: |
  ...
  if [ -z "$PROPS" ]; then
    echo '{"wakeAgent": false, "data": {"status": "not-configured", "hint": "set GA4_PROPERTIES (id[,label:id...]) or GA4_PROPERTY_ID in plugin-data/community-helper/config.env"}}'
    exit 0
  fi
...
against a recent release (check with the manager, or `release-announcement-
