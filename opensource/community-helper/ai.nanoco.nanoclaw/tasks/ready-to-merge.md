...
script: |
  ...
  exec > >(tee >(sed -u "s/^{/{\"_ts\":\"$(date -u +%FT%TZ)\",/" >> "$DATA/telemetry/ready-to-merge.jsonl" 2>/dev/null) 2>/dev/null)
...
Otherwise, hand your manager one short block per repo in `repos`:
...
- **`newly_ready`** — approved since your last report. Manager with these. This
