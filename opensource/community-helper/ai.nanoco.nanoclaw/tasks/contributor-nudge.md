...
script: |
  ...
  exec > >(tee >(sed -u "s/^{/{\"_ts\":\"$(date -u +%FT%TZ)\",/" >> "$DATA/telemetry/contributor-nudge.jsonl" 2>/dev/null) 2>/dev/null)
...
templated bot follow-up would destroy it. Hand it to your manager, and say the
window is time-boxed — this is worth the manager passing on now rather than
