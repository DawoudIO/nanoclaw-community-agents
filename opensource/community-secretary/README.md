# Community Secretary Agent Template
...
The lead agent (`opensource/community-manager`) answers people, and everything
...
```
opensource/community-secretary/
├── plugin.json
├── mcp.json
...
Gate scripts are **not** authored here. They live in `scripts/tasks/secretary/*.sh`
...
`plugin-data/community-secretary/config.env` in the group folder (the agent sees it
as `/workspace/agent/plugin-data/community-secretary/config.env`):
