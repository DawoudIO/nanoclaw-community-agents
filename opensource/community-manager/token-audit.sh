...
#
# Defaults to the most recently modified session transcript. Re-run this
# anytime the owner asks "where is our budget going" — it costs nothing to
# answer, unlike guessing from memory.
set -euo pipefail
...
for f in /workspace/agent/plugin-data/community-manager/*.jsonl /workspace/agent/plugin-data/community-manager/*.json; do
  [ -f "$f" ] || continue
  bytes=$(wc -c < "$f")
