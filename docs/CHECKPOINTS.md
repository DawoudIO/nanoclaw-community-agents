...
owns 11 of the 21 tasks was absent, misconfigured, or quietly billing to the
...
| 14 | **Host Ollama is up and the model is pulled** | `ollama ps` on the host; confirm `llama3.2` is present | The model is loaded and the host has headroom. This is the local tier's foundation — 11 of the 21 tasks are dead without it |
