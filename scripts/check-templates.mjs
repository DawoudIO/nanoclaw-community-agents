 ...
 *
 * VENDORED, UNMODIFIED, from nanocoai/nanoclaw-templates (scripts/check-templates.mjs)
 * — the canonical template registry. Kept here so these four templates can be
 * checked against the same contract the upstream registry enforces, before an
 * install discovers a problem. Re-copy it rather than editing it; a local edit
 * would let us drift from the contract while still reporting OK.
 */
