...
exec > >(tee >(sed -u "s/^{/{\"_ts\":\"$(date -u +%FT%TZ)\",/" >> "$DATA/telemetry/release-announcement-watch.jsonl" 2>/dev/null) 2>/dev/null)
...
if [ -z "$REPOS" ]; then
  echo '{"wakeAgent": false, "data": {"status": "not-configured", "hint": "set RELEASE_WATCH_REPOS (or COMMUNITY_REPOS) in plugin-data/community-manager/config.env"}}'
  exit 0
fi
TMP=$(mktemp -d)
i=0
for REPO in $REPOS; do
  (
    ...
    printf '{"repo": "%s", "status": "new-release", "baseline_file": "plugin-data/community-manager/last-announced-release-%s.txt", "release": %s}\n' "$REPO" "$SAFEREPO" "$RELEASE" > "$TMP/$i.json"
  ) &
  i=$((i+1))
done
wait
