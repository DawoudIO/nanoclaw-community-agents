...
| OneCLI dashboard address | George checks in remotely via Tailscale (old install ran at `http://100.68.197.18:10254`) — give the welcome interview the sandbox host's Tailscale IP, not `127.0.0.1`, so dashboard links the agent sends actually resolve. Confirm the fresh sbx install's OneCLI gateway version satisfies the `/v1` API check in `docs/onecli-upgrades.md` before relying on credential flows — the old install ran gateway v1.45.0, which predates OneCLI's v2.0.0 line |
