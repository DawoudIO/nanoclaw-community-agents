# ChurchCRM Community Manager Setup — Native Installation
...
- Subscription with API window for cloud Claude (Haiku-4.5 for Local Ops by default)
...
**Then**:
1. Select `opensource/community-manager` template
...
Choose: `opensource/community-manager` template, build locally.
...
**Lead agent workspace** (`groups/community-manager/`):
- `plugin-data/community-manager/config.env` — shell vars (COMMUNITY_REPOS, GITHUB_BOT_USERNAME)
- `plugin-data/community-manager/project-config.md` — prose config (owner DM, repo map, channels, goals, analytics, tone, target audience)
- `plugin-data/community-manager/public-actions.log` — ledger of public replies/comments
- `plugin-data/community-manager/social-metrics-history.jsonl` — weekly follower counts
- `plugin-data/community-manager/owner-instructions.jsonl` — owner DM ledger (ack #N tracking)

**Sub-agent workspaces** (same structure, separate directories):
- `plugin-data/community-secretary/config.env` + `project-config.md`
...
**Fix**: Set Community Manager (the lead agent) as the default handler. The agent will intelligently route to specialists based on channel tier and question type.
